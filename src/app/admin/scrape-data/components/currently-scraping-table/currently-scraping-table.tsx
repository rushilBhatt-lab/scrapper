...
export default function CurrentlyScrapingTable({ jobs }: { jobs: JobType[] }) {
	...
	const filteredItems = React.useMemo(() => {
		...
		if (hasSearchFilter) {
			filteredUsers = filteredUsers.filter((user) => user.url.toLowerCase().includes(filterValue.toLowerCase()));
		}
	...
	const renderCell = React.useCallback((user: JobType, columnKey: React.Key) => {
	...
	}, [filterValue, onSearchChange, statusFilter, jobs.length, onRowsPerPageChange, onClear]);
